...
function checkAvailability() {
  ...
  setTimeout(() => {
    ...
    if (isOverlap) {
      ...
      // Fungsi untuk menambahkan padding agar tanda ':' sejajar
      function padRight(str, length) {
        return str + " ".repeat(length - str.length);
      }

      const labelLength = 10; // panjang kolom label agar rata

      // 💬 Pesan WA rata dan monospace
      const messageText = 
`\`\`\`
🕹️ FORM PEMESANAN
─────────────────
${padRight("Nama", labelLength)}: ${nameVal}
${padRight("Konsol", labelLength)}: ${consoleVal}
${padRight("Room", labelLength)}: ${roomVal}
